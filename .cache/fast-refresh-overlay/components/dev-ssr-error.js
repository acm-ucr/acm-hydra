	...
	return (
		<Overlay>
			...
			<Body>
				...
				>
					React Components in Gatsby must render both successfully in
					the browser and in a Node.js environment. When we tried to
					render your page component in Node.js, it errored.
				</p>
				<h2>Source</h2>
				...
				<p>
					If you fixed the error, saved your file, and want to retry
					server rendering this page, please reload the page.
				</p>
				<button
					onClick={() => reloadPage()}
				...
				<h2 style={{ marginTop: `var(--space)` }}>
					Skip Server Render
				</h2>
				<p>
					If you don't wish to fix the SSR error at the moment, press
					the button below to reload the page without attempting to do
					SSR.
				</p>
				<button
					onClick={() => skipSSR()}
				...
				<Footer>
					<span data-font-weight='bold'>Note:</span> This error will
					show up during "gatsby build" so it must be fixed before
					then. SSR errors in module scope, e.g. outside of your own
					React components can't be skipped.
				</Footer>
			</Body>
		</Overlay>
	);
