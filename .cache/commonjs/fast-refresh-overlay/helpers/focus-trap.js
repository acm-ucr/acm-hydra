...
var gif =
	"data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"; // https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img#attr-usemap
...
var invalidGif =
	"data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ"; // NOTE: https://github.com/medialize/ally.js/issues/35
// https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img#attr-usemap
...
var focusRedirectLegend = {
	...
	mutate: function mutate(element) {
		element.innerHTML =
			'<legend>legend</legend><input tabindex="-1"><input tabindex="0">'; // take care of focus in validate();
...
function isFocusRelevantRules() {
	...
	if (
		nodeName === "audio" &&
		(supports$1.focusAudioWithoutControls ||
	...
	if (
		nodeName === "video" &&
		(supports$1.focusVideoWithoutControls ||
			element.hasAttribute("controls"))
	) {
		return true;
	}
...
function getMapByName(name, _document) {
	...
	var map = _document.querySelector(
		'map[name="' + (0, _css.default)(name) + '"]'
	);
...
function getImageOfArea(element) {
	...
	return (
		_document.querySelector(
			'img[usemap="#' + (0, _css.default)(map.name) + '"]'
		) || null
	);
}
...
function queryFocusableStrict() {
	...
	var walker = _document.createTreeWalker(
		...
		strategy === "all"
			? PossiblyFocusableFilter
			: createFilter(_isFocusable), // deprecated, but IE requires it
		false
	);
...
function insertElementsAtOffsets(list, insertions) {
	...
	insertions.forEach(function (insertion) {
		...
		var args = [insertion.offset + inserted, remove].concat(
			insertion.elements
		);
		...
		},
		{
			...
			value: function _injectHosts(elements) {
				Object.keys(this.hosts).forEach(function (_sortingId) {
					...
					this.elements[_sortingId] = this._merge(
						_list,
						_elements,
						_context
					);
