...
const reducer = (state, event) => {
	switch (event.action) {
		...
		case `SHOW_GETSERVERDATA_ERROR`: {
			return { ...state, getServerDataError: event.payload };
		}
...
function DevOverlay({ children }) {
	...
	const hasErrors =
		...
		hasGraphqlErrors ||
		hasDevSsrError ||
		hasGetServerDataError; // This component has a deliberate order (priority)
	...
	const ErrorComponent = () => {
		...
		if (hasGetServerDataError) {
			return /*#__PURE__*/ React.createElement(
				_getserverdataError.GetServerDataError,
				{
