...
class DevLoader extends _loader.BaseLoader {
	constructor(asyncRequires, matchPaths) {
		const loadComponent = chunkName => {
			...
			return this.asyncRequires.components[chunkName]()
		...
		if (!(0, _isEqual.default)(newPageData, cachedPageData)) {
			...
			if (pageDataDbCacheKey === `/404.html`) {
				this.notFoundPagePathsInCaches.forEach(notFoundPath => {
					const previousPageDataEntry =
						this.pageDataDb.get(notFoundPath);
	...
	handleStaleServerDataMessage() {
		...
		for (const [key, value] of this.pageDataDb) {
			...
			if (
				value !== null &&
				value !== void 0 &&
				(_value$payload = value.payload) !== null &&
				_value$payload !== void 0 &&
				(_value$payload$result = _value$payload.result) !== null &&
				_value$payload$result !== void 0 &&
				_value$payload$result.serverData
			) {
	...
	reFetchServerData(pagePath) {
		this.fetchPageDataJson({
			...
			if (
				updated &&
				pagePath === (0, _normalizePagePath.default)(location.pathname)
			) {
