...
var _ssrSyncRequires = _interopRequireDefault(
	require("$virtual/ssr-sync-requires")
);
...
const getStats = publicDir => {
	if (cachedStats) {
		...
		cachedStats = JSON.parse(
			_fs.default.readFileSync(
				_path.default.join(publicDir, `webpack.stats.json`),
				`utf-8`
			)
		);
		return cachedStats;
...
async function staticPage(
	...
	if (error) {
		postBodyComponents.push([
			...
			/*#__PURE__*/ _react.default.createElement(
				...
				/*#__PURE__*/ _react.default.createElement("h2", null, "File:"),
				/*#__PURE__*/ _react.default.createElement(
					"p",
					null,
					error.source,
	...
	const generateBodyHTML = async () => {
		...
		const getPageData = pagePath => {
			...
			const absolutePageDataPath = _path.default.join(
				publicDir,
				pageDataPath
			);

			const pageDataJson = _fs.default.readFileSync(
		...
		let scriptsAndStyles = (0, _lodash.flatten)(
			[`commons`].map(chunkKey => {
				...
				const namedChunkGroups = (0, _lodash.get)(
					stats,
					`namedChunkGroups`
				);
		...
		scriptsAndStyles = (0, _lodash.uniqBy)(
			scriptsAndStyles,
			item => item.name
		);
		...
		class RouteHandler extends _react.default.Component {
			render() {
				...
				const props = {
					...
					...pageData.result,
					params: {
						...(0, _findPath.grabMatchParams)(
							this.props.location.pathname
						),
						...(((_pageData$result = pageData.result) === null ||
						_pageData$result === void 0
							? void 0
							: (_pageData$result$page =
									_pageData$result.pageContext) === null ||
				...
				if (
					_ssrSyncRequires.default.ssrComponents[
						componentChunkName
					] &&
					!isClientOnlyPage
				) {
					pageElement = /*#__PURE__*/ _react.default.createElement(
						_ssrSyncRequires.default.ssrComponents[
							componentChunkName
						],
						props
				...
				const wrappedPage = (0, _apiRunnerSsr.apiRunner)(
					`wrapPageElement`,
					{
						element: pageElement,
						props,
		...
		const routerElement = /*#__PURE__*/ _react.default.createElement(
			...
			/*#__PURE__*/ _react.default.createElement(
				"div",
				_routeAnnouncerProps.RouteAnnouncerProps
			)
		);

		const bodyComponent = (0, _apiRunnerSsr.apiRunner)(
			`wrapRootElement`,
			{
